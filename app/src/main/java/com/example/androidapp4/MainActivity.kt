...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        searchEditText = findViewById(R.id.searchEditText); searchButton = findViewById(R.id.searchButton); longTitleCheckBox = findViewById(R.id.longTitleCheckBox)
        progressBar = findViewById(R.id.progressBar); statusTextView = findViewById(R.id.statusTextView); podcastListView = findViewById(R.id.podcastListView)
        selectedPodcastTextView = findViewById(R.id.selectedPodcastTextView); selectedPodcastArtworkImageView = findViewById(R.id.selectedPodcastArtworkImageView); selectedPodcastInitialTextView = findViewById(R.id.selectedPodcastInitialTextView); episodeTitleTextView = findViewById(R.id.episodeTitleTextView)
        subscribeButton = findViewById(R.id.subscribeButton); playButton = findViewById(R.id.playButton); playbackSeekBar = findViewById(R.id.playbackSeekBar); currentTimeTextView = findViewById(R.id.currentTimeTextView); durationTextView = findViewById(R.id.durationTextView)
        discoverTabButton = findViewById(R.id.discoverTabButton); subscribedTabButton = findViewById(R.id.subscribedTabButton); listSectionTitleTextView = findViewById(R.id.listSectionTitleTextView)
        ...
        // The same button starts, pauses and resumes the current episode.
    ...
    private fun selectPodcast(podcast: Podcast) {
        stopPlayback(false); selectedPodcast = podcast; selectedPodcastTextView.text = podcast.title; episodeTitleTextView.text = "Tap Play Latest to load an episode"
        selectedPodcastInitialTextView.text = podcast.title.firstOrNull()?.uppercaseChar()?.toString() ?: "P"; selectedPodcastInitialTextView.visibility = View.VISIBLE
        selectedPodcastArtworkImageView.load(podcast.artworkUrl) { crossfade(true); listener(onSuccess = { _, _ -> selectedPodcastInitialTextView.visibility = View.INVISIBLE }, onError = { _, _ -> selectedPodcastInitialTextView.visibility = View.VISIBLE }) }
    ...
    private fun searchPodcasts(term: String) {
        ...
        executor.execute { try { val podcasts = parsePodcastResults(downloadText(URL("https://itunes.apple.com/search?term=${URLEncoder.encode(term, "UTF-8")}&media=podcast&entity=podcast&limit=25")), filter); runOnUiThread { searchResults = podcasts; showingSubscribed = false; updateTabs(); showResults(podcasts, "Discover"); statusTextView.text = "${podcasts.size} podcasts found - tap one to select it."; setLoading(false) } } catch (_: Exception) { runOnUiThread { statusTextView.text = "Could not load podcasts. Check your internet connection."; podcastListView.adapter = null; setLoading(false) } } }
    }
    ...
    private fun updateTabs() { discoverTabButton.backgroundTintList = android.content.res.ColorStateList.valueOf(android.graphics.Color.parseColor(if (showingSubscribed) "#151C23" else "#123447")); subscribedTabButton.backgroundTintList = android.content.res.ColorStateList.valueOf(android.graphics.Color.parseColor(if (showingSubscribed) "#123447" else "#151C23")); discoverTabButton.setTextColor(android.graphics.Color.parseColor(if (showingSubscribed) "#94A3B8" else "#7DD3FC")); subscribedTabButton.setTextColor(android.graphics.Color.parseColor(if (showingSubscribed) "#7DD3FC" else "#94A3B8")) }
    ...
    private inner class PodcastAdapter(podcasts: List<Podcast>) : ArrayAdapter<Podcast>(this, R.layout.podcast_list_item, podcasts) {
        override fun getView(position: Int, convertView: View?, parent: ViewGroup): View {
            ...
            initial.text = podcast.title.firstOrNull()?.uppercaseChar()?.toString() ?: "P"; initial.visibility = View.VISIBLE; badge.visibility = if (isSubscribed(podcast)) View.VISIBLE else View.GONE
            artwork.load(podcast.artworkUrl) { crossfade(true); listener(onSuccess = { _, _ -> initial.visibility = View.INVISIBLE }, onError = { _, _ -> initial.visibility = View.VISIBLE }) }; return row
        }
    }

    /** Adds or removes one podcast and immediately refreshes the correct list. */
    private fun toggleSubscription(podcast: Podcast) {
        val list = loadSubscriptions().toMutableList(); val existing = list.indexOfFirst { it.collectionId == podcast.collectionId }
        if (existing >= 0) {
            list.removeAt(existing); saveSubscriptions(list); statusTextView.text = "Unsubscribed from ${podcast.title}"; subscribeButton.text = "Subscribe"
            if (showingSubscribed) {
                // Keep the message visible, but remove the podcast from the library immediately.
                podcastListView.adapter = PodcastAdapter(list)
                selectedPodcast = null; selectedPodcastTextView.text = "No podcast selected"; episodeTitleTextView.text = "Select a podcast to start listening"; subscribeButton.isEnabled = false; playButton.isEnabled = false; stopPlayback(false)
                listSectionTitleTextView.text = "Subscribed"
            } else podcastListView.adapter = PodcastAdapter(searchResults)
        } else {
            list.add(podcast); saveSubscriptions(list); statusTextView.text = "Subscribed to ${podcast.title}"; subscribeButton.text = "Unsubscribe"
            if (showingSubscribed) podcastListView.adapter = PodcastAdapter(list) else podcastListView.adapter = PodcastAdapter(searchResults)
        }
    }

    private fun isSubscribed(podcast: Podcast) = loadSubscriptions().any { it.collectionId == podcast.collectionId }
    ...
    private fun updateSubscribeButton(podcast: Podcast) { subscribeButton.text = if (isSubscribed(podcast)) "Unsubscribe" else "Subscribe" }
    ...
    private fun loadLatestEpisode(podcast: Podcast) {
        ...
        executor.execute { try { val results = JSONObject(downloadText(URL("https://itunes.apple.com/lookup?id=${podcast.collectionId}&entity=podcastEpisode&limit=5"))).getJSONArray("results"); var audioUrl: String? = null; var episodeTitle = "Latest episode"; for (i in 0 until results.length()) { val item = results.getJSONObject(i); if (item.optString("wrapperType") == "podcastEpisode" && item.optString("episodeUrl").isNotBlank()) { audioUrl = item.optString("episodeUrl"); episodeTitle = item.optString("trackName", episodeTitle); break } }; val url = audioUrl; val title = episodeTitle; runOnUiThread { setLoading(false); playButton.isEnabled = true; if (selectedPodcast?.collectionId != podcast.collectionId) return@runOnUiThread; if (url == null) { episodeTitleTextView.text = "No playable episode available"; statusTextView.text = "No playable episode was returned for ${podcast.title}." } else { episodeTitleTextView.text = title; playAudio(url, podcast.title, title) } } } catch (_: Exception) { runOnUiThread { setLoading(false); playButton.isEnabled = true; episodeTitleTextView.text = "Could not load episode"; statusTextView.text = "Could not load an episode for ${podcast.title}." } } }
    }
        ...
        player = ExoPlayer.Builder(this).build().also { p ->
            p.addListener(object : Player.Listener {
                override fun onPlaybackStateChanged(state: Int) {
                    when (state) {
                        Player.STATE_READY -> { playButton.isEnabled = true; durationTextView.text = formatTime(p.duration); if (p.isPlaying) statusTextView.text = "Playing $podcastTitle: $episodeTitle" }
                        Player.STATE_ENDED -> { p.seekTo(0); p.pause(); playbackSeekBar.progress = 0; currentTimeTextView.text = "0:00"; playButton.text = "Replay"; statusTextView.text = "Episode finished: $episodeTitle" }
                    }
                }
                // ExoPlayer reports play/pause changes here even when STATE_READY does not change.
                override fun onIsPlayingChanged(isPlaying: Boolean) {
                    if (p.playbackState == Player.STATE_READY) playButton.text = if (isPlaying) "Pause" else "Resume"
                }
    ...
    private fun stopPlayback(showMessage: Boolean) { player?.release(); player = null; resetProgress(); if (::playButton.isInitialized) { playButton.text = "Play Latest"; playButton.isEnabled = selectedPodcast != null }; if (showMessage && ::statusTextView.isInitialized) statusTextView.text = "Playback stopped." }
